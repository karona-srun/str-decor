...
@section('title-page', __('app.item_sub_group'))
        ...
        <div class="col-md-12">
            <div class="card card-outline card-primary">
                <div class="card-header">
                    <h3 class="card-title">{{ __('app.list_item_sub_group') }}</h3>
                    <div class="card-tools">
                        ...
                        @can('Item Sub Group Create')
                            <a href="{{ url('item-sub-group/create') }}" class="btn btn-primary"> <i class=" fas fa-plus"></i>
                                {{ __('app.btn_add') }}</a>
                        @endcan
                    </div>
                </div>
                    ...
                    <table id="datatable" class="table table-bordered table-striped" width="100%">
                        ...
                        <tbody>
                            @foreach ($items as $i => $item)
                                <tr>
                                    <td>{{ ++$i }}</td>
                                    <td>{{ $item->ItemGroup->item_group_name ?? 'N/A' }}</td>
                                    <td>{{ $item->item_sub_group_code }}</td>
                                    <td>{{ $item->item_sub_group_name }}</td>
                                    <td>
                                        @can('Item Sub Group Create')
                                            <a href="{{ route('item-sub-group.edit', $item->id) }}"
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $(".deleteProductCategory").click(function() {
                var id = $(this).data("id");
                $('.formDelete').attr('action', 'item-sub-group/' + id);
            });
